...
class User {
    constructor(fullname, adresse, email, cin, num_tel, password, img, age, id, resetCode, resetCodeExpiration) {
        this.id = id;
    ...
    static getUser(identifier) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (document != null) {
                    const user = new User(document.fullname, document.adresse || "", document.email || "", document.cin || "", document.num_tel || "", document.password, document.img, document.age, document._id.toString(), document.resetCode, document.resetCodeExpiration);
    ...
    static getUserById(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const user = yield db.collection("users").findOne({ _id: objectId });
                if (user != null) {
                    return new User(user.fullname, user.adresse, user.email, user.cin, user.num_tel, user.password, user.img, user.age, user._id.toString());
                }
                else {
                    return User.empty;
                }
